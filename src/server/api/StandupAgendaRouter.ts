                ...
                try {
                    ...
                    if (response && response.accessToken) {
                        ...
                        try {
                            ...
                            const chatId = Buffer.from(req.params.meetingId, "base64")
                                .toString("ascii")
                                .replace(/^0#|#0$/g, "");
                            // get chat details
                            const chat = await Axios.get<Chat>(
                                `https://graph.microsoft.com/beta/chats/${chatId}`,
                                authHeader
                            );
                            // get meeting detail (via chat detail)
                            const onlineMeetings = await Axios.get(
                                `https://graph.microsoft.com/v1.0/me/onlineMeetings?$filter=JoinWebUrl eq '${chat.data.onlineMeetingInfo?.joinWebUrl}'`,
                                authHeader
                            );
                            ...
                            if (onlineMeetings?.data?.value?.length > 0) {
                                ...
                                console.error(
                                    "bad data returned from online meeting request: ",
                                    onlineMeetings
                                );
                                throw new Error(
                                    "500: bad data returned from online meeting request"
                                );
                            }
                        } catch (err) {
                            // \get meeting detail
                            throw new Error(`error getting meeting detail: ${err.message}`);
                        }
                    } else {
                        // \use OBO access token to call MS graph for meeting detail
                    ...
                    // \get OBO token
                    throw new Error(`obo token acq error: ${err.message}`);
                }
            } catch (err) {
                // \validate token
